...
const scrapeCryptoPanic = () => new Promise((resolve, reject) => {

  // Get scrape urls.
  getCryptoPanicScrapeURLS(`${config.api.url}api/posts/?auth_token=${config.api.token}`, (urls) => {
    getCPCurrencyDataFromURLArray(urls, (cpData) => {
      console.log('done');
      return resolve(cpData);
    });
  });

  return reject(`Could not fetch CP Data.`);


});

scrapeCryptoPanic().then(data => console.log(data));
...
const getCPCurrencyDataFromURL = (url, currencies) => new Promise((resolve, reject) => {
...
function getCryptoPanicScrapeURLS(url, callback, carryArray){
  ...
  request.get(url, (err, response, body) => {
    ...
    setTimeout(() => {
      getCryptoPanicScrapeURLS(body.next, callback, carryArray);
    }, 500);
  });

};

function asyncSum(arr, sum){
  if (!sum) sum = 0;
  console.log(arr);
  var ci = arr.shift();
  return new Promise((resolve, reject) => {
    ...
    if (arr.length !== 0) return asyncSum(arr, sum);
    return sum;
  });
}
